...
import { GitHubUser, GitHubUserDetails, GitHubRepo } from '../models/github-user.model';
@Injectable({
  providedIn: 'root'
})
export class GithubApiService {

  constructor(private http: HttpClient, @Inject('GITHUB_API_URL') private apiUrl: string) { }

  getUsers(since: number): Promise<GitHubUser[]> {
    return new Promise((resolve, reject) => {
      //resolve(mockDataUsers);
      this.http.get<GitHubUser[]>(this.apiUrl + "/users" + (since > 0 ? "?since=" + since : "")).subscribe(result => resolve(result));
    });

  }
  getUser(login: string): Promise<GitHubUserDetails> {
    return new Promise((resolve, reject) => {
      //resolve(mockDataUser);
      this.http.get<GitHubUserDetails>(this.apiUrl + "/users/" + login).subscribe(result =>
        this.getUserRepos(result.repos_url).then(repos => {
          result.repos = repos;
          resolve(result);
        })
      );
  ...
  getUserRepos(url: string): Promise<GitHubRepo[]> {
    return new Promise((resolve, reject) => {
      this.http.get<GitHubRepo[]>(url).subscribe(result => resolve(result))
    });
  }
}
const mockDataUser = {
  ...
  updated_at: "2019-10-24T04:23:11Z"
};
const mockDataUsers = [
  {
    "login": "mojombo",
